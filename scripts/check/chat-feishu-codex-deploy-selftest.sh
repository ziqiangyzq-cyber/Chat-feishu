...
if bash "${repo_root}/scripts/deploy/chat-feishu-codex.sh" install --root "${test_root}/unsafe" --no-activate --home / \
  --binary "${test_root}/binary-v1" --codex-binary "${test_root}/codex" --env-file "${test_root}/service.env" --config-file "${test_root}/config.json" >/dev/null 2>&1; then
...
if bash "${repo_root}/scripts/deploy/chat-feishu-codex.sh" install --root "${test_root}/bad-root" --no-activate \
  --binary "${test_root}/binary-v1" --codex-binary "${test_root}/bad-codex" --env-file "${test_root}/service.env" --config-file "${test_root}/config.json" >/dev/null 2>&1; then
  echo "Codex binary without app-server was accepted" >&2
  exit 1
fi
deploy install --binary "${test_root}/binary-v1" --codex-binary "${test_root}/codex" --env-file "${test_root}/service.env" --config-file "${test_root}/config.json"
...
bash "${repo_root}/scripts/deploy/chat-feishu-codex.sh" install --root "${fresh_root}" --no-activate \
  --binary "${test_root}/binary-v1" --codex-binary "${test_root}/codex" --env-file "${test_root}/service.env" --config-file "${test_root}/config.json"
