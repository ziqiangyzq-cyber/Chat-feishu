...
配置安全语义是 fail-closed：`maxAccessMode` 非空但无法归一化时降级为最严格的 `confirm` 并打 ERROR 日志；`workspaceRoots` 配置了但全部条目无效时按"拒绝所有工作区"生效；每个 root 会额外做 `EvalSymlinks`，把符号链接的真实路径一并纳入白名单（解析失败保留原值并告警）。
