...
> 6. managed provider bootstrap 由 wrapper 统一监督：initialize 最长等待 30 秒，期间持续排空并转发 stderr、保留 32 KiB 尾部诊断；失败会终止完整 provider 进程组。surface 对同一恢复目标连续三次同类失败后写入 durable 熔断状态并停止后台重试，只有显式 mode/target/retry 路径清除，避免 daemon 重启后再次进入永久恢复循环。
...
3. `thread/start` 会优先使用飞书显式 `/model` override 或已观察的原生模板；两者都没有时，注入本代 `config/read` 解析出的 bootstrap 默认模型。这样既保持用户显式选择优先，也兼容无法处理 `model:null`、会让新会话创建长期无响应的 Desktop app-server。成功的 `thread/start` / `thread/resume` 响应会把其必填 `result.model` 记到对应 thread；后续本地 UI 对同一 thread发出带有效 model 的 `turn/start` 时，再按事件顺序更新同一条 thread model 记录。这样 restart/resume 新响应会替换旧代模板值，而响应之后的本地显式切换仍能成为最新值。
