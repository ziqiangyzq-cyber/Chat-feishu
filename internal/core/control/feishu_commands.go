...
var feishuCommandSpecs = []feishuCommandSpec{
	...
	},
	{
		definition: FeishuCommandDefinition{
			...
			ArgumentFormNote: "输入 codex / claude / agy / grok / vscode；`normal` 仍兼容为 `codex`。",
			ArgumentSubmit:   "切换",
			Description:      "查看当前模式；bare `/mode` 会返回 codex / claude / agy / grok / vscode 切换卡片。",
			Examples:         []string{"/mode codex", "/mode claude", "/mode agy", "/mode grok", "/mode vscode", "/mode normal"},
			Options: []FeishuCommandOption{
				...
				commandOption("/mode", "mode", "agy", "agy", "切换到 headless 的 Antigravity 模式。"),
				commandOption("/mode", "mode", "grok", "grok", "切换到 headless 的 Grok 模式。"),
