...
var feishuCommandSpecs = []feishuCommandSpec{
	...
	},
	{
		definition: FeishuCommandDefinition{
			...
			ArgumentFormNote: "输入 codex / claude / agy / vscode；`normal` 仍兼容为 `codex`。",
			ArgumentSubmit:   "切换",
			Description:      "查看当前模式；bare `/mode` 会返回 codex / claude / agy / vscode 切换卡片。",
			Examples:         []string{"/mode codex", "/mode claude", "/mode agy", "/mode vscode", "/mode normal"},
			Options: []FeishuCommandOption{
