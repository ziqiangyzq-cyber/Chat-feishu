...
const (
	...
	RoleGrokBridge           Role = "grok_bridge"
)
...
func isWrapperMode(mode string) bool {
	switch mode {
	case "app-server", "claude-app-server", "agy-app-server", "grok-app-server":
		return true
	default:
		return false
	}
...
func Detect(args []string) (Decision, error) {
	...
	case "grok-bridge":
		return Decision{Role: RoleGrokBridge, Args: args[1:]}, nil
	case "wrapper":
		if len(args) < 2 {
			return Decision{}, usageError("wrapper requires app-server, claude-app-server, agy-app-server, or grok-app-server arguments")
		}
		if !isWrapperMode(args[1]) {
			return Decision{}, usageError("wrapper only supports app-server, claude-app-server, agy-app-server, or grok-app-server mode")
		}
